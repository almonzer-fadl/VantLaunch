...
export function ArticlePage({ data, slug }: { data: ArticleData; slug: string }) {
  return (
        ...
        <section className="border-t border-black/[0.06] px-6 py-12 sm:py-16">
          <div className="mx-auto max-w-3xl">
            <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-2xl border border-black/10 bg-white p-6 shadow-mid mb-10">
              ...
              <ol className="mt-3 space-y-1.5">{data.sections.map((s, i) => (<li key={i} className="text-sm text-[#74695B]">{i + 1}. {s.heading}</li>))}</ol>
            </motion.div>
            {data.sections.map((section, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className="mb-10">
                ...
                <div className="text-[#74695B] leading-relaxed space-y-4">{section.content.split("\n\n").map((p, j) => (<p key={j}>{p}</p>))}</div>
              </motion.div>
            ))}
            {data.faq.length > 0 && (
              <div className="rounded-2xl border border-black/10 bg-white p-6 shadow-mid mb-10">
                <h2 className="text-lg font-bold mb-4">Frequently Asked Questions</h2>
                <div className="space-y-3">{data.faq.map((item, i) => (<div key={i} className="border-b border-black/[0.04] last:border-0 pb-3"><h3 className="text-sm font-bold text-[#11100E]">{item.q}</h3><p className="mt-1 text-sm text-[#74695B]">{item.a}</p></div>))}</div>
              </div>
            )}
            <div className="flex items-center gap-3 text-sm text-[#74695B] mb-10"><Share2 className="h-4 w-4" /><span className="font-bold">Share this article</span></div>
            <div className="grid gap-8 sm:grid-cols-2 mb-10">
              <div><h3 className="text-xs font-bold uppercase tracking-[0.1em] text-[#74695B] mb-3">Related Articles</h3><div className="space-y-2">{data.related.map((r, i) => (<Link key={i} href={r.href} className="block text-sm font-medium text-[#004225] hover:text-[#11100E] transition-colors">{r.title}</Link>))}</div></div>
              <div><h3 className="text-xs font-bold uppercase tracking-[0.1em] text-[#74695B] mb-3">Related Resources</h3><div className="space-y-2">{data.resources.map((r, i) => (<Link key={i} href={r.href} className="block text-sm font-medium text-[#004225] hover:text-[#11100E] transition-colors">{r.title}</Link>))}</div></div>
            </div>
