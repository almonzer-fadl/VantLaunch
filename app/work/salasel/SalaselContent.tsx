  ...
  return (
    <div className="space-y-16">
      <section>
        ...
        <figure className="relative mt-12 overflow-hidden rounded-[1.75rem] border border-white/[0.1] bg-black-surface shadow-[0_40px_100px_-40px_rgba(0,0,0,0.85)]">
          ...
          <figcaption className="type-figure-caption border-t border-white/[0.06] bg-black-surface/95 px-5 py-4">
      ...
      <section className="space-y-5">
        ...
        <p className="type-case-overview-text">
          ...
          <strong className="font-semibold text-zinc-300">digitized procurement journeys</strong>,{' '}
          <strong className="font-semibold text-zinc-300">credit-enabled purchasing</strong>, and{' '}
          <strong className="font-semibold text-zinc-300">trustworthy fulfillment visibility</strong>{' '}
          from cart share to reconciliation.
        </p>
        <div className="glass-card rounded-3xl border border-white/[0.07] bg-white/[0.02] p-8">
          <div className="flex items-start gap-4">
            <Layers className="mt-1 h-8 w-8 shrink-0 text-white" aria-hidden />
            <div>
      ...
      <section>
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">
          {stakeholders.map(({ title, body, icon: Icon }) => (
            <div
              key={title}
              className="glass-card rounded-[1.75rem] border border-white/[0.06] bg-white/[0.02] p-7 transition-colors hover:border-white/25"
            >
        ...
        <div className="mt-8 overflow-x-auto rounded-[1.5rem] border border-white/[0.08]">
          <table className="w-full min-w-[640px] text-left text-sm">
            ...
            <tbody className="divide-y divide-white/[0.05]">
              {panels.map((row) => (
                <tr key={row.route} className="text-zinc-300 hover:bg-white/[0.02]">
                  <td className="px-5 py-4">
                    <p className="font-bold text-white">{row.panel}</p>
                    <p className="type-table-meta mt-1">{row.description}</p>
                  </td>
                  <td className="px-5 py-4 font-medium text-white/90">{row.route}</td>
                  <td className="px-5 py-4 font-medium text-zinc-400">{row.guard}</td>
                  <td className="px-5 py-4 font-mono text-[13px] text-zinc-500">{row.model}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section>
        <h2 className="type-case-section-heading-inline">
          <Database className="h-8 w-8 text-white" aria-hidden />
      ...
      <section>
        ...
        <ul className="mt-6 list-none space-y-3 text-[17px] text-zinc-400">
      ...
      <section>
        <h2 className="type-case-section-heading-inline">
          <BookOpen className="h-8 w-8 text-white" aria-hidden />
        ...
        <ul className="mt-6 grid gap-3 text-sm font-medium text-zinc-400 sm:grid-cols-2">
      ...
      <section>
        ...
        <pre className="mt-6 overflow-x-auto rounded-2xl border border-white/[0.08] bg-black/55 p-6 text-[13px] leading-relaxed text-zinc-300">
{`app/
├── Filament/
...
docs/{complete,srs,files,...}`}
      ...
      <section>
        <h2 className="type-case-section-heading-inline">
          <TestTube className="h-8 w-8 text-white" aria-hidden />
          Validation & tooling
        </h2>
        <div className="mt-8 glass-card rounded-3xl border border-white/[0.07] p-8">
          <ul className="space-y-4 type-prose-list">
            <li>
              ...
              <span className="font-mono text-sm text-white/90">php artisan test</span>, suites
              for feature flows,{' '}
              <span className="font-mono text-sm text-zinc-500">php artisan test --coverage</span> when wired.
            </li>
            <li>
              <strong className="font-semibold text-slate-200">Formatting:</strong> PSR-12 + Laravel{' '}
              <span className="font-mono text-sm text-zinc-500">vendor/bin/pint</span>.
            </li>
            <li>
      ...
      <section className="rounded-[2rem] border border-white/25 bg-white/[0.08] px-8 py-12 text-center">
...
function DocPathRow({ path, caption }: { path: string; caption: string }) {
  return (
    <li className="flex flex-col rounded-xl border border-white/[0.05] bg-white/[0.02] px-4 py-3">
      <code className="text-[13px] text-white">{path}</code>
    ...
    <section>
      <details className="group rounded-[1.75rem] border border-white/[0.09] bg-white/[0.02] open:bg-white/[0.03]">
        <summary className="type-summary-toggle">
          ...
          <span className="rounded-full bg-white/10 px-4 py-1 text-xs font-semibold text-zinc-300 group-open:hidden">
            Expand
          </span>
          <span className="hidden rounded-full bg-white/10 px-4 py-1 text-xs font-semibold text-zinc-300 group-open:inline">
            Collapse
          </span>
        </summary>
        <div className="border-t border-white/[0.06] px-8 pb-8 pt-2">
          ...
          <pre className="mt-3 overflow-x-auto rounded-xl border border-white/[0.08] bg-black/55 p-4 text-[13px] text-zinc-300">{`git clone <repository-url>
cd salasel
composer install && npm install`}</pre>

          <h3 className="type-playbook-h3">Environment</h3>
          <pre className="mt-3 overflow-x-auto rounded-xl border border-white/[0.08] bg-black/55 p-4 text-[13px] text-zinc-300">{`cp .env.example .env
...
# MAIL_* — configure transactional provider`}</pre>
          ...
          <pre className="mt-3 overflow-x-auto rounded-xl border border-white/[0.08] bg-black/55 p-4 text-[13px] text-zinc-300">{`php artisan migrate
php artisan db:seed
php artisan shield:generate --all`}</pre>

          <h3 className="type-playbook-h3">Run locally</h3>
          <pre className="mt-3 overflow-x-auto rounded-xl border border-white/[0.08] bg-black/55 p-4 text-[13px] text-zinc-300">{`npm run build       # prod assets once
npm run dev         # vite dev server
php artisan serve
php artisan queue:work`}</pre>

          <h3 className="type-playbook-h3">Handy Laravel / Filament tooling</h3>
          <pre className="mt-3 overflow-x-auto rounded-xl border border-white/[0.08] bg-black/55 p-4 text-[13px] text-zinc-300">{`php artisan make:migration create_table_name
